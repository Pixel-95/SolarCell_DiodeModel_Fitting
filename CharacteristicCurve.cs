...
namespace SolarCell_DiodeModel_Fitting
{
    ...
    public class CharacteristicCurve
    {
        ...
        /// </summary>
        public double currentPhoto { get; private set; }
        /// <summary>
        ...
        {
            this.temperature = temperature;
            this.experimentalData = experimentalData.Select(e => (e.voltage, -Math.Sign(experimentalData.FirstOrDefault().current) * e.current, e.power, e.area, e.efficiency)).ToList();
            coefficientOfDetermination = double.NaN;
        }
        ...
        {
            experimentalData = experimentalData.OrderBy(d => d.voltage).ToList();
            var voltages = experimentalData.Select(e => e.voltage).ToArray();
            var currents = Smoothing.SavitskyGolay(-Math.Sign(experimentalData.First().current) * Vector.Create(experimentalData.Select(e => e.current).ToArray()), 9, 3).ToArray();
            ...
            fitterShunt.Fit();
            double Rsh = 1 / fitterShunt.BestFitParameters[1];
            if (Rsh < 0)
                Rsh = 1e30;
            double Iph = -fitterShunt.BestFitParameters[0];
            ...
            fitterSeries.Fit();
            double Rs = Math.Max((currents.Where(c => c > 0).Count() > 2 ? 1 : 0.5) / fitterSeries.BestFitParameters[1], 1e-30);

            // n
            double n = 2 * Voc.value;

            // I0
            double I0 = (Iph - Voc.value / Rsh) / (Math.Exp((MainWindow.e * Voc.value) / (n * MainWindow.kB * temperature)) - 1);

            currentPhoto = Iph;
            currentSaturation = I0;
            diode1IdealityFactor = n;
            Rseries = Rs;
            Rshunt = Rsh;
            ...
            for (int i = 0; i < orderedExperimentalData.Select(d => d.voltage).ToArray().Length; i++)
                SSres += ChiSquared(orderedExperimentalData.Select(d => d.voltage).ToArray()[i],
                    orderedExperimentalData.Select(d => d.current).ToArray()[i], new double[] { currentPhoto, currentSaturation, diode1IdealityFactor, Rseries, Rshunt });
            ...
            return new double[] { currentPhoto, currentSaturation, diode1IdealityFactor, Rseries, Rshunt };
        ...
        public double[] FitWithGivenParameters(double[] initGuess, bool[] fitThisParameter)
        {
            var orderedExperimentalData = experimentalData.OrderBy(d => d.voltage).ToList();

            // fit
            NonlinearCurveFitter fitter = new NonlinearCurveFitter();
            fitter.Curve = new IVcurve(temperature, initGuess, fitThisParameter);
            ...
            currentPhoto = index_Iph == -1 ? initGuess[0] : fitter.BestFitParameters[index_Iph];
            currentSaturation = index_I0 == -1 ? initGuess[1] : fitter.BestFitParameters[index_I0];
            diode1IdealityFactor = index_n == -1 ? initGuess[2] : fitter.BestFitParameters[index_n];
            Rseries = index_Rs == -1 ? initGuess[3] : fitter.BestFitParameters[index_Rs];
            if (Rseries < 0)
                Rseries = double.NaN;
            Rshunt = index_Rsh == -1 ? initGuess[4] : fitter.BestFitParameters[index_Rsh];
            if (Rshunt < 0)
                Rshunt = double.NaN;
        ...
        /// <param name="Rseries">series resistor of the microcell in Ohm</param>
        /// <param name="Rshunt">resistor of the short cut within the microcell, where current can flow back to the back contact in Ohm</param>
        public double[] ExecuteFit(double currentPhoto = double.NaN, double currentSaturation = double.NaN, double diode1IdealityFactor = double.NaN,
            double Rseries = double.NaN, double Rshunt = double.NaN)
        {
            // Initial guess ————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————
            double[] initialGuess = GetInitGuess();
            ...
            return new double[] { this.currentPhoto, this.currentSaturation, this.diode1IdealityFactor, this.Rseries, this.Rshunt };
                ...
                double.IsNaN(diode1IdealityFactor) || double.IsNaN(Rseries) || double.IsNaN(Rshunt))
            {
                ...
                Console.WriteLine("kB = " + MainWindow.kB);
                Console.WriteLine("e = " + MainWindow.e);
                Console.WriteLine("T = " + temperature);

                // Get I0
                if (double.IsNaN(currentSaturation))
                    this.currentSaturation = this.currentPhoto / (Math.Exp((MainWindow.e * Voc)
                        / (this.diode1IdealityFactor * MainWindow.kB * temperature)) - 1);
                else
                    this.currentSaturation = currentSaturation;
            ...
            double? specialRs = null, double? specialRp = null)
        {
            ...
            {
                return (-Iph + I0 * (Math.Exp(MainWindow.e * voltage / (n * MainWindow.kB * temperature)) - 1)) * factorToCurrent;
            }
            ...
            {
                return (-Iph + I0 * (Math.Exp(MainWindow.e * voltage / (n * MainWindow.kB * temperature)) - 1) + voltage / Rsh) * factorToCurrent;
            }
            ...
            {
                double inputL = I0 * MainWindow.e * Rs / (n * MainWindow.kB * temperature)
                    * Math.Exp(MainWindow.e * ((Iph + I0) * Rs + voltage) / (n * MainWindow.kB * temperature));
                return (n * MainWindow.kB * temperature / (MainWindow.e * Rs) * MainWindow.LambertW(inputL) - Iph - I0) * factorToCurrent;
            }
            ...
            {
                double inputL = MainWindow.e * I0 * Rs * Rsh / ((Rs + Rsh) * n * MainWindow.kB * temperature)
                    * Math.Exp(MainWindow.e * Rsh * (Rs * (Iph + I0) + voltage) / ((Rs + Rsh) * n * MainWindow.kB * temperature));
                return (n * MainWindow.kB * temperature / (MainWindow.e * Rs) * MainWindow.LambertW(inputL)
            ...
            double? specialRs = null, double? specialRp = null)
        {
            ...
            if (double.IsNaN(Rs) && double.IsNaN(Rsh))
            {
                return (MainWindow.e / (n * MainWindow.kB * temperature)
                    * I0 * Math.Exp(MainWindow.e * voltage / (n * MainWindow.kB * temperature))) * factorToCurrent;
            }
            ...
            {
                return (MainWindow.e / (n * MainWindow.kB * temperature)
                    * I0 * Math.Exp(MainWindow.e * voltage / (n * MainWindow.kB * temperature))
            ...
            {
                double inputLambertFunction = MainWindow.e * I0 * Rs / (n * MainWindow.kB * temperature)
                    * Math.Exp(MainWindow.e * ((Iph + I0) * Rs + voltage)
                    / (n * MainWindow.kB * temperature));
                return (MainWindow.e * MainWindow.LambertW(inputLambertFunction)
                    / (MainWindow.LambertW(inputLambertFunction) + 1)) * factorToCurrent;
            }
            ...
            {
                double inputLambert = MainWindow.e * I0 * Rs * Rsh
                    / ((Rs + Rsh) * n * MainWindow.kB * temperature)
                    * Math.Exp(MainWindow.e * Rs * (Rsh * (Iph + I0) - voltage)
                    / ((Rs + Rsh) * n * MainWindow.kB * temperature)
                    + MainWindow.e * voltage / (n * MainWindow.kB * temperature));
                return ((1 / Rs - 1 / (Rs + Rsh)) * MainWindow.LambertW(inputLambert) / (MainWindow.LambertW(inputLambert) + 1)
            ...
            double? specialRs = null, double? specialRp = null)
        {
            ...
            else if (double.IsNaN(Rsh))
            {
                ...
                double e = MainWindow.e;
                double kB = MainWindow.kB;
                double U = voltage;
                double exp = Math.Exp(e * ((Iph + I0) * Rs - U) / (n * kB * T));
                double f = e * I0 * Rs / (n * kB * T) * exp;
                double W = MainWindow.LambertW(f);
        ...
        private double GetVoltageOpenCircuit()
        {
            ...
            {
                // return Voc
                return (diode1IdealityFactor * MainWindow.kB * temperature / MainWindow.e * Math.Log(currentPhoto / currentSaturation + 1)) * factorToVoltage;
            }
            else
            {
                // Determine exponent in the Lambert function (might be to big for the exponent of a double)
                double exp = MainWindow.e * Rshunt * (currentPhoto + currentSaturation) / (diode1IdealityFactor * MainWindow.kB * temperature);
                ...
                if (exp > 300)
                {
                    ...
                    double w1LOGw2 = Math.Log(currentSaturation * MainWindow.e * Rshunt / (diode1IdealityFactor * MainWindow.kB * temperature)) + exp;
                    lambert = w1LOGw2 - Math.Log(w1LOGw2) + Math.Log(w1LOGw2) / w1LOGw2;
                }
                else
                {
                    // exact formula of W(x)
                    lambert = MainWindow.LambertW(currentSaturation * MainWindow.e * Rshunt / (diode1IdealityFactor * MainWindow.kB * temperature)
                        * Math.Exp(exp));
                }

                // return Voc
                return (Rshunt * (currentPhoto + currentSaturation) - diode1IdealityFactor * MainWindow.kB * temperature / MainWindow.e * lambert) * factorToVoltage;
            }
        }
        ...
        private double GetVoltageMaximumPowerPoint()
        {
            ...
            {
                return ((n * MainWindow.kB * temperature) / MainWindow.e
                    * (MainWindow.LambertW((Iph / currentSaturation + 1) * Math.E) - 1)) * factorToVoltage;
            }
            ...
            else if (double.IsNaN(Rseries) && !double.IsNaN(Rshunt)) // Diode model without Rs and with Rp        ->  I = I(Rp)
            {
                ...
                Func<double, double> dPdV = V => -Iph + I0 * Math.Exp(MainWindow.e * V / (n * MainWindow.kB * temperature))
                * (1 + MainWindow.e * V / (n * MainWindow.kB * temperature)) - I0 + V / Rp;

                // Specity delegate derivate functions for newton method (no numerical derivation needed)
                Func<double, double> dP2dV2 = V => I0 * MainWindow.e / (n * MainWindow.kB * temperature)
                * Math.Exp(MainWindow.e * V / (n * MainWindow.kB * temperature))
                * (2 + MainWindow.e * V / (n * MainWindow.kB * temperature)) + 1 / Rp;

                // return Vmpp
                return (MainWindow.FindRootViaNewtonMethod(dPdV, dP2dV2, 0.8 * GetVoltageOpenCircuit() / factorToVoltage, 1e-10)) * factorToVoltage;
            }
            ...
            else if (!double.IsNaN(Rseries) && double.IsNaN(Rshunt)) // Diode model with Rs and without Rp        ->  I = I(Rs)
            {
                ...
                Func<double, double> inputL = V => I0 * Rs * MainWindow.e / (n * MainWindow.kB * temperature)
                    * Math.Exp(MainWindow.e * ((Iph + I0) * Rs + V) / (n * MainWindow.kB * temperature));
                Func<double, double> dinputLdV = V => I0 * Rs * Math.Pow(MainWindow.e / (n * MainWindow.kB * temperature), 2)
                    * Math.Exp(MainWindow.e * ((Iph + I0) * Rs + V) / (n * MainWindow.kB * temperature));
                Func<double, double> fraction = V => V * dinputLdV(V) / (inputL(V) * (MainWindow.LambertW(inputL(V)) + 1));
                Func<double, double> dPdV = V => -n * MainWindow.kB * temperature / (MainWindow.e * Rs)
                    * MainWindow.LambertW(inputL(V)) * (fraction(V) + 1) + Iph + I0;

                // derviation for newton method could be determined analytically but is very messy due to second derivative of W(x)

                // return Vmpp
                return (MainWindow.FindRootViaNewtonMethod(dPdV, 0.8 * GetVoltageOpenCircuit() / factorToVoltage, 1e-10)) * factorToVoltage;
            }
            ...
            else // Diode model with Rs and with Rp                                                               ->  I = I(Rs, Rp)
            {
                ...
                */
                ...
                double a = 1 / (Rs + Rp);
                double b = (Rs + Rp) * n * MainWindow.kB * temperature / (MainWindow.e * Rs);
                double c = Rp * (Iph + I0);
                double d = MainWindow.e * I0 * Rs * Rp / ((Rs + Rp) * n * MainWindow.kB * temperature);
                double g = MainWindow.e * Rs / ((Rs + Rp) * n * MainWindow.kB * temperature);
                double k = Rp * (Iph + I0);
                double l = MainWindow.e / (n * MainWindow.kB * temperature);

                // Specity certain delegate functions, where the last one will be searched for its root (maximum in power is root in derivation)
                Func<double, double> inputL = V => d * Math.Exp(g * (k - V) + l * V);
                Func<double, double> dinputLdV = V => d * (l - g) * Math.Exp(g * (k - V) + l * V);
                Func<double, double> P = V => a * (b * MainWindow.LambertW(inputL(V)) + V - c) * V;
                Func<double, double> fraction = V => dinputLdV(V) * MainWindow.LambertW(inputL(V)) / (inputL(V) * (MainWindow.LambertW(inputL(V)) + 1));
                Func<double, double> dPdV = V => a * (b * fraction(V) + 1) * V + a * (b * MainWindow.LambertW(inputL(V)) + V - c);
